...
$(document).ready(() => {
    ...
    // (location.pathname == "/private/kontrolna_tabla.html") ? $.getScript(Url.CDN[0] + "gh/BaleshSrle/misc/private/BS_CP_main.min.js") : $.getScript(Url.CDN[0] + "gh/BaleshSrle/misc/private/BS_site.min.js");
    $("head").each((i, e) => {
        ...
        $(e).append($("<link>").attr({ "rel": "dns-prefetch", "href": Url.CDN[0] }).on("error", function () { this.href = Url.CDN[1] }), $("<link>").attr({ "rel": "dns-prefetch", "href": GoogleFontsUrl[1] }), $("<link>").attr({ "rel": "preconnect", "href": Url.CDN[0], "crossorigin": "anonymous" }).on("error", function () { this.href = Url.CDN[1] }), $("<link>").attr({ "rel": "preconnect", "href": GoogleFontsUrl[0] }), $("<link>").attr({ "rel": "preconnect", "href": GoogleFontsUrl[1], "crossorigin": "anonymous" }), $("<link>").attr({ "rel": "prefetch", "href": Url.Badges[0] }).on("error", function () { this.href = Url.Badges[1] }), $("<link>").attr({ "rel": "stylesheet", "href": Url.CDN[0] + "npm/bootstrap@4/dist/css/bootstrap.min.css", "media": "all", "crossorigin": "anonymous" }).one("error", function () { this.href = Url.CDN[1] + "ajax/libs/bootstrap/4.6.2/css/bootstrap.min.css" }), $("<link>").attr({ "rel": "stylesheet", "href": GoogleFontsUrl[0] + "/css2?family=Ubuntu:ital,wght@0,400;0,700;1,400;1,700&display=swap", "media": "all", "crossorigin": "anonymous" }), $("<link>").attr({ "rel": "stylesheet", "href": Url.CDN[0] + "npm/bootstrap-icons@1/font/bootstrap-icons.min.css", "media": "all", "crossorigin": "anonymous" }).one("error", function () { this.href = Url.CDN[1] + "ajax/libs/bootstrap-icons/1.13.1/font/bootstrap-icons.min.css" }), $("<link>").attr({ "rel": "icon", "href": Url.CDN[0] + "gh/BaleshSrle/baleshsrle.github.io/logo.png" }));
    ...
    $("img.githubBadge").each((i, e) => {
        ...
        $img.attr({ "src": Url.Badges[0] + "github/v/" + Type[0] + "/" + Package + "?filter=" + Filter + "&logo=" + Logo + "&logoColor=" + LogoColor + "&logoSize=" + LogoSize + "&label=" + Label + "%20%40%20GitHub&labelColor=" + LabelColor + "&color=" + Color, "alt": "Badge for GitHub Package " + Package, "loading": (i === 0) ? "eager" : "lazy", "crossorigin": "anonymous" }).one("error", function () { this.src = Url.Badges[1] + "github/" + Type[1] + "/" + Package + Stable + "?icon=" + Logo + "&label=" + Label + "%20%40%20GitHub&labelColor=" + LabelColor + "&color=" + Color; }).wrap($("<a></a>").attr({ "href": Url.GitHub + Package + "/" + Type[0] + "s" + Latest, "hreflang": "en", "target": "_blank", "rel": "external" }));
    });
    ...
    $("img.websiteBadge").each((i, e) => {
        ...
        $img.attr({ "src": Url.Badges[0] + "website?url=" + URL + "&logo=" + Logo + "&logoColor=" + LogoColor + "&logoSize=" + LogoSize + "&label=" + Label + "&labelColor=" + Color + "&cacheSeconds=1800", "alt": "Shields.io Website Badge", "loading": (i === 0) ? "eager" : "lazy", "crossorigin": "anonymous" });
    });
    document.normalize();
});